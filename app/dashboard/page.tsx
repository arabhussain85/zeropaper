...
import AddReceiptDialog from "@/components/add-receipt-dialog"
import { getReceipts, type Receipt } from "@/services/receipt-service"
...
export default function DashboardPage() {
  ...
  // Filter receipts by active category
  const filteredReceipts = receipts.filter((receipt) => activeCategory === "all" || receipt.category === activeCategory)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <motion.button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg whitespace-nowrap transition-colors ${
                activeCategory === "all" ? "bg-black text-white" : "bg-white text-gray-900"
              }`}
              variants={itemVariants}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
      ...
      >
        ...
        <motion.div className="flex justify-between items-center mb-6" variants={itemVariants}>
          ...
          <p className="text-xl font-semibold">
            ...
            {filteredReceipts.length > 0
              ? `${getCurrencySymbol(filteredReceipts[0]?.currency || "USD")}${filteredReceipts.reduce((sum, receipt) => sum + receipt.price, 0).toFixed(2)}`
              : "$0.00"}
          </p>
        </motion.div>
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#1B9D65]"></div>
        ...
        {!isLoading && filteredReceipts.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-500 mb-6">
              {activeCategory !== "all"
                ? `No ${activeCategory} receipts found. Try selecting a different category.`
                : "Add your first receipt to get started"}
            </p>
            <motion.button
              onClick={() => setIsAddReceiptOpen(true)}
              className="px-4 py-2 bg-[#1B9D65] text-white rounded-lg hover:bg-[#1B9D65]/90"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              Add Receipt
